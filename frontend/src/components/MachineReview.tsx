...
export default function MachineReview() {
  ...
  const handleDeleteTask = async () => {
    if (!window.confirm('Are you sure you want to delete this scheduled assignment?')) return;

    setLoading(true);
    try {
      const backendUrl = import.meta.env.VITE_BACKEND_URL || 'http://localhost:8000';
      const token = localStorage.getItem('access_token');
      const userId = localStorage.getItem('user_id') || task.engineerId;

      const response = await fetch(`${backendUrl}/api/shed2machine/${machine._id}/assignment/${userId}/${task.inspectionDate}`, {
        method: 'DELETE',
